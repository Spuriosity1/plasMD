...
int main() {
  ...
  real_t wavek = 4;   // wavenumber
  exafmm_t::ModifiedHelmholtzFmm fmm(P, ncrit, wavek);

  /* step 3: Build and balance the octree.
   */
  start("Build Tree");
  exafmm_t::NodePtrs<real_t> leafs, nonleafs;
  exafmm_t::Nodes<real_t> nodes;
   ...
   */
  std::vector<real_t> potential(ntargets);
  std::vector<real_t> gradient_x(ntargets);
  std::vector<real_t> gradient_y(ntargets);
  std::vector<real_t> gradient_z(ntargets);

  for (int i=0; i<leafs.size(); ++i) {
    exafmm_t::Node<real_t>* leaf = leafs[i];
